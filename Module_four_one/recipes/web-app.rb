...
include_recipe 'Dev_CentOS7_AT_HW::default'
...
mod_jk_file = 'tomcat-connectors-1.2.42-src.tar.gz'
...
remote_file "#{mod_jk_path}#{mod_jk_file}" do	
  source "http://www-us.apache.org/dist/tomcat/tomcat-connectors/jk/tomcat-connectors-1.2.42-src.tar.gz"
  mode '0755'
...
bash "compile_mod_jk" do
  ...
  code <<-EOH
    cd #{mod_jk_path}
    tar -xvf #{mod_jk_file} 
    cd  #{mod_jk_path}/tomcat-connectors-1.2.42-src/native  
